 ...
 * 入力チャンクを {@link parseKey} で操作へ写像してフォーカス移動・完了トグルを行う。
 *
 * トグルは現在フォーカス中の todo の状態に応じて core の純粋関数 `complete` /
 * `reopen` を選び、`repo.update` で永続化したのち一覧を読み直して再描画する。
 * 入力チャンクは `AsyncIterable` から逐次取り出すため、トグルの I/O は次のキー処理を
 * ブロックし、書き込みが重ならない。`quit`（`q` / Ctrl-C）またはストリーム終端で
 * ループを抜け、raw mode 解除とカーソル再表示を `finally` で必ず行う。
 *
